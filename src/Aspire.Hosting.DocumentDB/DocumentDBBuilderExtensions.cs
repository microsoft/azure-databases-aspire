...
public static class DocumentDBBuilderExtensions
{
    ...
    /// produce the same command for a remote daemon. Two shapes otherwise produce that set: a
    /// mount whose whole region falls inside the candidate's region, which makes every path under
    /// that mount an alias; and a mount whose region contains the candidate's, which makes the one
    /// path that lands on the candidate's region an alias, together with everything under it and
    /// each of its ancestors down to the mount point — a data directory above the scratch region
    /// contains it just as surely as one inside it.
    /// </para>
    /// <para>
...
#pragma warning restore CS0618
    ...
    /// <summary>
    /// What this package knows about the image the resource will really run: the seal a run takes
    /// before the orchestrator prepares anything, and the live model when there is none.
    /// </summary>
    /// <remarks>
    ...
    /// and the check that the model still agrees with it are allowed to use. Outside a run no seal
    /// is ever taken and this is exactly <see cref="ResolveModelImage"/> — a publish reads the
    /// model at serialization time, which is the only image a publish has.
    /// </remarks>
        ...
        System.Collections.Immutable.ImmutableArray<DockerfileBuildAnnotation> build) : IResourceAnnotation
    {
        ...
        /// re-derivation of it. That includes the <c>registry/</c> prefix, the <c>@sha256:</c>
...
#pragma warning restore ASPIREPIPELINES001
    ...
    /// </para>
    /// <para>
    /// One hook seals the whole model, not one hook per resource: it is registered with
    /// <c>TryAddLifecycleHook</c>, which deduplicates on service and implementation type, and it
    /// walks the model rather than closing over a resource.
    /// </para>
    /// <para>
...
#pragma warning restore CS0618
    ...
    /// </para>
    /// <para>
    /// Outside a run there is no seal and this does nothing.
    /// </para>
    /// </remarks>
    private static void VerifyImageSeal(IResource resource)
    {
        ...
        throw new InvalidOperationException(
            ...
            $"this package sealed it: {change}. Aspire snapshots a container's image into the DCP " +
            $"container spec while it prepares resources, which is before endpoints are allocated " +
            $"and before the resource-start event is published, so a change made after the image " +
            $"it will run had already been settled lands in the application model but never " +
            $"reaches the container runtime: the container would run the sealed reference while " +
            $"this package judged the current one — its version floors, its data-directory " +
            $"interlock and its declared-volume advice would all describe an image that is not the " +
            $"one starting. The resource is failed instead. " +
            $"Sealed reference: '{DescribeReference(seal.Reference)}'. " +
            $"Current reference: '{DescribeReference(reference)}'. " +
            $"Recovery: choose the image while the application model is being built — " +
            $"WithImage(...), WithImageTag(...), WithImageRegistry(...), WithImageSHA256(...), " +
            $"WithDockerfile(...) — or at the latest from a BeforeStartEvent subscriber, rather " +
            $"than from a ResourceEndpointsAllocatedEvent or BeforeResourceStartedEvent subscriber " +
            $"or an IDistributedApplicationLifecycleHook that runs after this package's.");

        static string DescribeReference(string? reference) => reference ?? "<none>";
    }
    ...
    /// option grammar, and fails the resource when it carries an option that would add storage, set
    /// an environment variable this package owns, replace the entry point, or displace the image.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// same list, and a value a callback contributed is not visible from the call that registered
    /// it. <see cref="EnsureTerminalCallbackRunsLast{TAnnotation}"/> is what makes this callback
    /// that one, and there is one such callback for the package, so the storage rules and the
    /// OpenTelemetry wrapper judge the same line rather than competing for the last position.
    /// </para>
    /// <para>
    /// While the wrapper is required the list is resolved first and written back, so the exact
    /// strings validated here are the exact strings the runtime receives and Aspire does not
    /// evaluate a deferred value a second time. Otherwise nothing is resolved: a deferred token
    /// where the runtime reads an option name could be any of the refused options and is reported
    /// as such, which is the fail-closed answer that does not duplicate Aspire's own evaluation of
    /// a value that may be a secret.
    /// </para>
    /// <para>
        ...
        DistributedApplicationExecutionContext executionContext)
    {
        ...
        // Hand the runtime the very strings that were validated, so a deferred value is neither
        // resolved twice nor able to resolve to something else on the second reading.
        context.Args.Clear();
        foreach (var argument in resolved)
        {
    ...
    private static async Task<List<object>> ResolveContainerRuntimeArgumentsAsync(
        ...
        DistributedApplicationExecutionContext executionContext)
    {
        var resolved = new List<object>(context.Args.Count);
        ...
        IResourceBuilder<DocumentDBServerResource> builder)
    {
        ...
        guard.RuntimeCheckpoint = new ContainerRuntimeArgsCallbackAnnotation(async context =>
        {
            ...
            await ValidateContainerRuntimeArgumentsAsync(
                resource,
                context,
