...
internal enum DocumentDBRuntimeArgumentVerdict
{
    ...
    /// <summary>
    /// An option neither runtime documents, in a reading that cannot pass one through — see
    /// <see cref="DocumentDBRuntimeArgumentStrictness.Grammar"/> for the reading that can.
    /// </summary>
    UnknownOption,

    /// <summary>An option that requires a value and was given none.</summary>
    MissingValue,
}

/// <summary>
/// How much of the argument list the caller is answerable for.
/// </summary>
/// <remarks>
/// <para>
/// The grammar is the same either way; what differs is what happens to a token the grammar does
/// not contain. <see cref="Grammar"/> reads an unrecognised option as a flag, which is the
/// fail-closed direction for a rule that only has to see past it: a following <c>--mount</c> stays
/// visible, and an option a future runtime adds still reaches that runtime untouched.
/// <see cref="Strict"/> refuses it instead, which is what a caller owns when this package also owns
/// the container's command: an option nothing here models could consume the token after it, and the
/// wrapper's own command is what the token after it would be read against.
/// </para>
/// <para>
/// Nothing about the refusal names the token. An unknown option is reported by kind alone, because
/// its spelling is the caller's and so is its value.
/// </para>
/// </remarks>
internal enum DocumentDBRuntimeArgumentStrictness
{
    /// <summary>An unrecognised option is read as a flag and passed through.</summary>
    Grammar,

    /// <summary>An unrecognised option, or a required value that is missing, is refused.</summary>
    Strict,
}
...
internal static class DocumentDBContainerRuntimeArguments
{
    ...
    /// <summary>
    /// Every <c>docker run</c> and <c>podman run</c> option that consumes nothing after it.
    /// </summary>
    /// <remarks>
    /// The other half of the arity union. It is not needed to read the line — an option that takes
    /// no value leaves the next token to be read as an option, which is what an unrecognised token
    /// already does — but it is what makes "this option is documented by neither runtime"
    /// answerable, which <see cref="DocumentDBRuntimeArgumentStrictness.Strict"/> needs. Options
    /// this package refuses are in the set too when their arity is nil, because their arity is what
    /// keeps a bare <c>--systemd</c> from swallowing the <c>--mount</c> behind it.
    /// </remarks>
    private static readonly HashSet<string> s_valuelessKnownOptions = new(StringComparer.Ordinal)
    {
        // Shared by both runtimes.
        "--detach", "-d", "--disable-content-trust", "--help", "--init", "--interactive", "-i",
        "--no-healthcheck", "--oom-kill-disable", "--privileged", "--publish-all", "-P",
        "--quiet", "-q", "--read-only", "--rm", "--sig-proxy", "--tty", "-t", "--use-api-socket",

        // Podman only.
        "--env-host", "--http-proxy", "--no-hostname", "--no-hosts", "--passwd", "--read-only-tmpfs",
        "--replace", "--rmi", "--rootfs", "--systemd", "--tls-verify", "--unsetenv-all",
    };

    /// <summary>
    /// The single-letter forms of <see cref="s_valuelessKnownOptions"/>, so a cluster such as
    /// <c>-it</c> is read letter by letter and an unrecognised letter is still recognisable as one.
    /// </summary>
    private static readonly HashSet<char> s_valuelessKnownShortOptions =
        [.. s_valuelessKnownOptions
            .Where(option => option.Length == 2 && option[0] == '-' && option[1] != '-')
            .Select(option => option[1])];

    /// <summary>
    /// Whether either runtime documents an option of this name.
    /// </summary>
    private static bool IsKnownOption(string name) =>
        s_valueTakingOptions.Contains(name) ||
        s_valuelessKnownOptions.Contains(name) ||
        s_storageOptions.Contains(name) ||
        s_scopedEnvironmentOptions.Contains(name) ||
        s_unscopedEnvironmentOptions.Contains(name) ||
        s_entrypointOptions.Contains(name) ||
        s_imageOptions.Contains(name);

    private static bool IsKnownShortOption(char letter) =>
        s_valueTakingShortOptions.Contains(letter) || s_valuelessKnownShortOptions.Contains(letter);
    ...
    /// What to do with a token the grammar does not contain. See
    /// <see cref="DocumentDBRuntimeArgumentStrictness"/>.
    /// </param>
    internal static DocumentDBRuntimeArgumentFinding Read(
        IEnumerable<object> arguments,
        Func<string, string?> resolveOwnedVariable,
        DocumentDBRuntimeArgumentStrictness strictness = DocumentDBRuntimeArgumentStrictness.Grammar)
    {
        ...
        for (var index = 0; index < tokens.Count; index++)
        {
            ...
            if (token.StartsWith("--", StringComparison.Ordinal))
            {
                ...
                if (strictness == DocumentDBRuntimeArgumentStrictness.Strict && !IsKnownOption(name))
                {
                    // Neither the name nor the value is reported: both are the caller's.
                    return new(DocumentDBRuntimeArgumentVerdict.UnknownOption, null, null);
                }
            ...
            if (!ReadShortOptionCluster(token, resolveOwnedVariable, strictness, out var clusterFinding, out var clusterPending))
            {
        ...
        if (strictness == DocumentDBRuntimeArgumentStrictness.Strict &&
            expectOperand &&
            pendingOption is { } unsatisfied)
        {
            // The spelling is this class's own, so naming it carries nothing the caller wrote.
            return new(DocumentDBRuntimeArgumentVerdict.MissingValue, unsatisfied, null);
        }
    ...
    private static bool ReadShortOptionCluster(
        string token,
        Func<string, string?> resolveOwnedVariable,
        DocumentDBRuntimeArgumentStrictness strictness,
        ...
        out string? pendingOption)
    {
        ...
        for (var index = 1; index < token.Length; index++)
        {
            ...
            if (strictness == DocumentDBRuntimeArgumentStrictness.Strict &&
                !IsKnownShortOption(token[index]))
            {
                finding = new(DocumentDBRuntimeArgumentVerdict.UnknownOption, null, null);
                return false;
            }
