...
- Documented the DocumentDB storage contract in a new "Storage requirements" section of the configuration reference, with matching troubleshooting entries: the data directory must be writable, must be empty or hold an existing cluster, is owned by the container's `documentdb` user (so bind-mount contents change uid/gid on Linux), and is initialized only once. From `0.116.0` it also backs exactly one running container at a time, initialization markers live in `<data-path>/.documentdb-local/` and survive restarts and volume backups (so a failed partial initialization is not retried and corrected scripts require a fresh data directory), and runs that mount nothing on `/data` get an anonymous volume that is never reused and that container removal can strand. Mounting on `/data` — which both persistence helpers do by default — is the only way to suppress that volume, because neither Docker nor Aspire can un-declare an image volume; a non-default `targetPath` now logs a warning saying so, but only on images known to declare it. Version-dependent behaviour is now labelled throughout, so the guidance stays correct both before and after `0.116.0` becomes the default.
