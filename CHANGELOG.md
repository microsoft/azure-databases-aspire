...
### Fixed
- Runtime-command failures from the `WithOpenTelemetryMetrics(...)` safety parser no longer disclose resolved operands. A bare `WithContainerRuntimeArgs(...)` value is an image/rootfs operand to Docker and Podman, so a secret `ParameterResource` or credential-bearing `ReferenceExpression` could previously reach the runtime's own error text after being resolved. The terminal callback now rejects every positional operand, including one after `--`, with a value-free diagnostic before container creation. Unknown options, missing required values, and deferred-resolution failures are generic too; the original exception is deliberately not retained. Diagnostics may identify a known option or package-owned environment variable, but never include an image/rootfs operand, mount specification, environment value, URI, credential, deferred result, or partial failed resolution. Deferred values are still resolved exactly once.
- The runtime parser now covers the documented Docker/Podman `run` option union with explicit arity. Docker's `--read-only` and `--storage-opt`, plus Podman's `--secret`, `--image-volume`, `--init-path`, `--chrootdirs`, `--ipc`, `--read-only-tmpfs`, and `--systemd`, are treated as storage-changing; `--pod` and `--pod-id-file` are included because a shared pod IPC namespace can replace `/dev/shm`; `--rootfs` is treated as an image/rootfs override; `--env-host` and `--unsetenv-all` are uninspectable environment imports; and protected `--env-merge` and `--unsetenv` values are rejected by name. Podman's value-less `--env`/`-e` wildcard imports are rejected when their prefix can match any protected telemetry variable, while unrelated prefixes and assigned forms containing `=` remain unchanged. Split and inline forms, deferred options/values, the `--net` alias, and harmless Podman-only controls are covered. Value-less options do not consume a following dangerous inline option. The existing Docker mount, entrypoint, environment, ShellExecution, scratch-isolation, custom-image, and non-wrapper behavior is unchanged.
- Closed the remaining runtime-command bypasses around the `WithOpenTelemetryMetrics(...)` compatibility wrapper. The package's terminal container-runtime callback now resolves the final Docker runtime arguments exactly once and parses Docker's split, `--option=value`, and attached short-option grammar from a typed option table, so a value supplied to an unrelated option is not mistaken for a mount. Raw `--mount`, `--volume`/`-v`, `--volume-driver`, `--tmpfs`, `--volumes-from`, and `--use-api-socket` options are rejected while the wrapper is required because they bypass `ContainerMountAnnotation` and can remount the data volume or a host path at a scratch root after the wrapper is generated. Raw `--entrypoint` is rejected because it can replace the verified `/bin/bash`, and protected `--env`/`-e` overrides plus `--env-file` are rejected because they can contradict the telemetry or `DATA_PATH` decision outside the model. Deferred runtime values keep single-evaluation semantics: the checkpoint replaces each provider with the string it validated, so Aspire does not resolve it again. Diagnostics identify only the option class and never repeat a potentially sensitive value; harmless runtime arguments and custom images remain unchanged.
- `ContainerResource.ShellExecution=true` is now rejected whenever the OpenTelemetry compatibility wrapper is required. Aspire DCP applies that switch after the terminal argument callback and replaces the verified `-c <script> -- ...` list with `-c "<joined arguments>"`, which handed `/bin/bash` a nested `-c` and prevented DocumentDB from starting. The wrapper now requires `ShellExecution` to stay `null` or `false`, records its effective value in the terminal seal, and re-checks it at the uncached container-runtime and manifest checkpoints, including after an early configuration read. Resources that do not need the wrapper keep their existing shell-execution behavior.
- Bind-backed telemetry scratch candidates are now conservative across Docker daemon boundaries. A bind source is resolved by the daemon or publish host and may traverse symbolic links unavailable on the machine building the model, so two lexically different source paths cannot prove physical disjointness. Whenever `DATA_PATH` and a candidate are both bind-backed, that candidate is skipped and the wrapper tries `/tmp`, `/var/tmp`, then `/dev/shm`; it fails clearly when every candidate aliases the data directory or cannot be proven independent. Named volumes are still compared exactly, different storage types stay usable, and the generated manifest no longer depends on local host-path or symlink resolution.
