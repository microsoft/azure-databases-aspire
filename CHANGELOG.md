...
### Fixed
- The `WithPostgresEndpoint()` credential floor and the `Pg18` publish floor are now judged on the image the container runtime is actually given. Aspire composes the image reference once, in `ContainerCreator.PrepareObjects()`, and writes it into the DCP container spec before endpoints are allocated and before `BeforeResourceStartedEvent` is published, so every checkpoint this package can reach runs after that decision: a `.WithImageTag(...)` applied from a `ResourceEndpointsAllocatedEvent` subscriber was accepted by the floors while the prepared container still launched the superseded image — including the pre-`0.112.0` image the credential floor exists to refuse, and the reverse case where the application model reports a newer release than the container runs. A run now seals the resource's composed image reference and its `DockerfileBuildAnnotation` build origin from a lifecycle hook, which is strictly later than every `BeforeStartEvent` subscriber and still guaranteed to precede DCP; every version-dependent decision is judged on the sealed value, and an image or build origin changed after the seal fails the run with a message naming both references. Choosing the image anywhere during model building — `WithImageTag(...)`, `WithImageRegistry(...)`, `WithImage(...)`, `WithImageSHA256(...)`, `WithDockerfile(...)`, `WithPostgresVersion(...)`, `WithDocumentDBVersion(...)`, in any order, including from a `BeforeStartEvent` subscriber — is unchanged, as are the custom-image, unrecognised-tag and digest-pin carve-outs. Publish mode is deliberately not sealed: no container is prepared, so the manifest checkpoint keeps judging the model at serialization time and published manifests are byte-for-byte unchanged.
- The `WithPostgresEndpoint()` credential floor and the `Pg18` publish floor are now judged on the image the resource will actually run. Both were ordinary `BeforeResourceStartedEvent` subscribers, and a subscriber registered after `AddDocumentDB` runs after them: nothing is published between that point and the container's creation, so a `.WithImageTag("pg17-0.111.0")` or `.WithImageTag("pg18-0.113.0")` applied there started a container that cannot authenticate with the Aspire-generated `postgresql://` URI, or one whose tag has never existed on GHCR. The two floors are now stated once and re-applied where Aspire caches nothing — a container-runtime-arguments callback immediately before each container creation, and a manifest publishing callback while the resource is actually serialized — so a publish is refused with the same actionable message rather than emitting a manifest naming an unusable image. `ExcludeFromManifest()` remains the boundary, the published manifest is byte-for-byte unchanged for a legal resource, and the documented carve-outs are unchanged: custom images, tags outside the `pg{NN}-X.Y.Z` grammar and digest pins carry no version to judge, early or late.
