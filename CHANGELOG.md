...
### Fixed
- `WithLogLevel(...)` now sets the gateway's canonical `DOCUMENTDB_LOG_LEVEL` variable in addition to the legacy `LOG_LEVEL` variable. This preserves existing image compatibility while making `Debug` and other configured levels effective in the DocumentDB `0.116.0` gateway.
