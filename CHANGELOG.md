...
- Bind-mount sources are now compared as host paths rather than as written, so `/srv/documentdb`, `/srv/documentdb/.` and `/srv/documentdb/../documentdb` are recognised as one host directory. Aspire records a rooted bind source verbatim, so those spellings previously reached the shared-data-directory check unequal and two resources could take one directory — silently, on images at or below `0.114.0`, which have no interlock. Symbolic links are deliberately not resolved: that would depend on the state of the host filesystem at model-build time.
