...
- Documented and enforced the version-specific storage contract: writable, valid PostgreSQL data directories for every supported image, plus the declared `/data` volume, exclusive data-directory lock, and marker-backed one-shot initialization introduced in `0.116.0`.

### Fixed
- Read-only DocumentDB data storage now fails immediately instead of producing a misleading startup failure. `WithDataVolume(isReadOnly: true)` and `WithDataBindMount(..., isReadOnly: true)` throw an `ArgumentException`, and a read-only volume or bind mount placed on the data path with the raw Aspire `WithVolume`/`WithBindMount` APIs fails the resource start with an `InvalidOperationException`. A read-only data directory can never work: the DocumentDB entrypoint takes ownership of it and `initdb` must change its permissions before PostgreSQL can create the cluster. `WithInitData(...)` and `WithTlsCertificate(...)` still mount their inputs read-only, which is correct and unaffected.
- Sharing one data directory between two DocumentDB resources in the same application model now fails at start with an explanatory `InvalidOperationException` naming both resources.
...
- `WithDataVolume(targetPath: ...)` now validates and normalizes the target path, rejecting empty, relative, and container-root values before startup.
