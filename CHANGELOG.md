...
- Documented the complete DocumentDB `0.116.0` storage contract in a new "Storage requirements" section of the configuration reference, with matching troubleshooting entries: the data directory must be writable, backs exactly one running container at a time, is owned by the container's `documentdb` user (so bind-mount contents change uid/gid on Linux), and is initialized only once. Initialization markers live in `<data-path>/.documentdb-local/` and survive restarts and volume backups, so a failed partial initialization is not retried and corrected scripts require a fresh data directory. Runs that mount nothing on `/data` get an anonymous volume that is never reused and that container removal can strand; mounting on `/data` — which both persistence helpers do by default — is the only way to suppress it, because neither Docker nor Aspire can un-declare an image volume. A non-default `targetPath` now logs a warning saying so.
