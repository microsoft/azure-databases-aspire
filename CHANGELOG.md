...
- `WithDataVolume(targetPath: ...)` now validates and canonicalizes the target path. Empty, whitespace, and relative paths are rejected with an `ArgumentException`, and repeated separators, `.` and `..` segments are resolved as the container runtime resolves them, so `/data/`, `//data`, and `/foo/../data` all land on the image's declared `/data` volume. A target that resolves to the container root (`/data/..`) or reaches above it (`/../data`) is rejected: the runtime refuses `destination can't be '/'`.
- The data-directory guards now judge the path the container will really use, after resolving it. That path is the effective `DATA_PATH` the resource's environment pipeline produces at start, so a raw `WithEnvironment("DATA_PATH", ...)` — a literal, a parameter, or a value computed in a callback — participates in the read-only, duplicate-mount, and shared-data-directory rules with the documented "last call wins" precedence against `WithDataVolume(...)`/`WithDataBindMount(...)`, and every comparison runs on canonicalized container paths. Previously the guards compared the storage helpers' own target after trimming trailing slashes only, so an alias such as `/foo/../data` looked like a different directory from `/data` even though the runtime mounts both on the same destination; on images with no data-directory interlock that let two resources take one data directory concurrently. Only `DATA_PATH` is resolved: every other environment value, the password among them, is left untouched.
