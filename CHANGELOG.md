...
- `WithDataVolume(targetPath: ...)` now validates the target path. Empty, whitespace, relative, and container-root paths are rejected with an `ArgumentException` instead of being forwarded to the container runtime as an opaque mount error, and a trailing slash is normalized so `/data/` still lands on the image's declared `/data` volume.
