...
- The runtime parser now covers the documented Docker/Podman `run` option union with explicit arity. Docker's `--read-only` and `--storage-opt`, plus Podman's `--secret`, `--image-volume`, `--chrootdirs`, `--ipc`, `--read-only-tmpfs`, and `--systemd`, are treated as storage-changing; `--pod` and `--pod-id-file` are included because a shared pod IPC namespace can replace `/dev/shm`; `--rootfs` is treated as an image/rootfs override; `--env-host` and `--unsetenv-all` are uninspectable environment imports; and protected `--env-merge` and `--unsetenv` values are rejected by name. Split and inline forms, deferred options/values, the `--net` alias, and harmless Podman-only controls are covered. The existing Docker mount, entrypoint, environment, ShellExecution, scratch-isolation, custom-image, and non-wrapper behavior is unchanged.
