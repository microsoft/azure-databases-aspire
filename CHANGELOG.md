...
### Fixed
- The official `documentdb-local` image is now recognised however the reference spells the boundary between registry and repository. `ContainerImageAnnotation` keeps a registry prefix and a repository apart, but Aspire only joins them with a separator and never re-splits them, so `WithImage("ghcr.io/documentdb/documentdb/documentdb-local", "pg17-0.116.0").WithImageRegistry(null)` publishes and runs exactly the reference the default spelling does. Comparing the image field on its own called that a custom image and withheld everything that is true of the official one: the `/data` volume declaration and the data-directory `flock`, the `WithPostgresEndpoint()` credential floor, the `Pg18` publish floor, and the `WithOpenTelemetryMetrics(...)` compatibility configuration and its publish rejection. Classification now normalizes the composed reference, so every spelling of one image is treated alike, and a digest is read whether it arrives through `WithImageSHA256(...)` or inline as `repository@sha256:...` (a `:` is a tag only in the last path segment, so a registry port is never mistaken for one). The private-mirror contract is unchanged and now covers inline spellings such as `contoso.azurecr.io/documentdb/documentdb-local` and `localhost:5000/documentdb/documentdb-local`: only the registry may differ, and the repository identity stays exact. A leading path that is not a registry host is part of the repository, so `evil/documentdb/documentdb-local` and `ghcr.io/evil/documentdb/documentdb-local` remain custom, as does a reference that leaves the registry annotation in place while repeating it in the image field, which composes an unresolvable `ghcr.io/documentdb/ghcr.io/...`. Dockerfile builds remain custom whatever their image text says.
