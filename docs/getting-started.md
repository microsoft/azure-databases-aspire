...
- **Data is ephemeral.** By default, DocumentDB stores data inside the container, so replacing the container loses it. Up to and including `0.114.0` that is a directory in the container's writable layer; from `0.116.0` the image declares `/data` as a container volume, so each run instead gets an anonymous volume that is never reused and that container removal can strand. Use `WithDataVolume()` or `WithDataBindMount()` to persist data across restarts — on `0.116.0` and later they also suppress that anonymous volume by mounting on the same path. A data directory must be writable and must be empty or hold an existing cluster. From `0.116.0` it is also initialized only once and backs only one running container at a time; on `0.114.0` and earlier the requested initialization runs again on every start, so seed scripts used with persisted storage must be idempotent. See [Storage requirements](configuration.md#storage-requirements) for details.
