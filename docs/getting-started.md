...
- **Data is ephemeral.** By default, DocumentDB stores data in a container-runtime-managed anonymous volume that is not reused between runs and that container removal can strand. When the container is replaced, you lose that data. Use `WithDataVolume()` or `WithDataBindMount()` to persist data across restarts — they also suppress the anonymous volume. A data directory must be writable, backs only one running container at a time, and is initialized only once. See [Storage requirements](configuration.md#storage-requirements) for details.
