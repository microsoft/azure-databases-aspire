...
**Cause:** Aspire writes a container's `build` object, image, entrypoint and mounts before it evaluates the environment callbacks, and its bindings after them. A `WithEnvironment(...)` callback that adds, removes or replaces a mount, re-points an endpoint, swaps the image, or adds, replaces or edits the Dockerfile build definition therefore changes the resource halfway through its own manifest entry: the fields already written describe the resource as it was, and the storage rules judged the resource as it became. The entry cannot be taken back, so the publish is failed and the partly written manifest is abandoned rather than completed.
